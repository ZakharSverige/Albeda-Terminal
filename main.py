            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                if not is_registered:
                    ...
                    if btn_start.collidepoint(event.pos) and selected_ip:
                        ...
                        if is_running: threading.Thread(target=vision_spam_worker, daemon=True).start()
                    elif search_rect.collidepoint(event.pos): search_active = True
                    elif vid_rect.collidepoint(event.pos):
                        webbrowser.open(f"https://www.youtube.com/watch?v={LAST_VIDEO_ID}")
                    else:
                        search_active = False
                        for i, s in enumerate(page_items):
                            if pygame.Rect(110, y_offset + (i*38), 780, 32).collidepoint(event.pos):
                                selected_ip = s['ip']

            if event.type == pygame.KEYDOWN:
                if not is_registered:
                    if event.key == pygame.K_BACKSPACE: user_nickname = user_nickname[:-1]
                    elif event.key == pygame.K_RETURN and len(user_nickname) >= 3:
                        is_registered = True
                        with open(PROFILE_PATH, "w", encoding='utf-8') as f: json.dump({"nickname": user_nickname}, f)
                    else: user_nickname += event.unicode
                elif search_active:
                    if event.key == pygame.K_BACKSPACE: search_query = search_query[:-1]
                    elif event.key == pygame.K_RETURN: search_active = False
                    else: search_query += event.unicode
                if event.key == pygame.K_F8 and selected_ip:
                    is_running = not is_running
                    if is_running: threading.Thread(target=vision_spam_worker, daemon=True).start()

        pygame.display.flip(); clock.tick(60)
except Exception as e: print(f"CRASH: {e}")
finally: app_running = False; pygame.quit(); os._exit(0)
